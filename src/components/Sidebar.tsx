...
export const Sidebar: React.FC<SidebarProps> = ({ currentScreen, onNavigate, dark = false }) => {
  ...
  return (
    ...
    >
      ...
      <div id="sidebar-tutor-profile" className="flex items-center gap-3 mb-2 px-2 pt-2">
      ...
      <button
        ...
        className="w-full bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] hover:opacity-95 text-white font-semibold py-2.5 px-4 rounded-xl shadow-md transition-all hover:scale-[0.98] active:scale-95 flex items-center justify-center gap-2 text-sm cursor-pointer"
      >
      ...
      <nav id="sidebar-nav-links" className="flex-1 flex flex-col gap-1.5 mt-2">
        <button
          id="sidebar-nav-home-btn"
          type="button"
          onClick={() => onNavigate('home')}
          className={`flex items-center gap-3 px-4 py-2.5 rounded-xl font-medium text-sm transition-all w-full text-left cursor-pointer ${
            currentScreen === 'home'
      ...
      {/* Footer Navigation - Help and Settings */}
      <div 
        id="sidebar-footer-actions"
        className={`mt-auto flex flex-col gap-1.5 pt-4 border-t ${dark ? 'border-white/10' : 'border-[#c7c4d7]/70'}`}
      >
        <button
          id="sidebar-footer-help-btn"
          type="button"
          onClick={() => onNavigate('adaptive')}
          className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all w-full text-left cursor-pointer ${
